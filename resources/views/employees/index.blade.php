            ...
            <tbody class="text-gray-600 text-sm">
                @forelse($employees as $employee)
                    <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                        {{-- In the view migration, 'id' is selected as 'employee_id' --}}
                        <td class="px-4 py-4 font-mono text-xs">#{{ $employee->employee_id }}</td>
                        
                        <td class="px-4 py-4 font-semibold text-gray-800">
                            {{-- The View already combined the names into 'full_name' --}}
                            {{ $employee->full_name }}
                        </td>
                        
                        <td class="px-4 py-4">
                            <span class="bg-blue-50 text-blue-600 px-2 py-1 rounded text-[11px] font-bold uppercase">
                                {{-- Accessing position_name directly from the View --}}
                                {{ $employee->position_name ?? 'N/A' }}
                            </span>
                        </td>
                        
                        <td class="px-4 py-4 text-gray-500">
                            {{-- Accessing department_name directly from the View --}}
                            {{ $employee->department_name ?? 'N/A' }}
                        </td>
                        
                        <td class="px-4 py-4">
                            <div class="flex justify-center items-center gap-3">
                                {{-- Use employee_id for the routes --}}
                                <a href="{{ route('employees.edit', $employee->employee_id) }}" class="text-blue-500 hover:text-blue-700 transition-colors" title="Edit">
                                    <i class="bi bi-pencil-square text-lg"></i>
                                </a>

                                <form action="{{ route('employees.destroy', $employee->employee_id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this employee?');" class="inline">
                                    @csrf
                                    @method('DELETE')
